...
def load_data_SIR_total(path='../data/History_country_2020_05_06.csv', name='中国', N=140005000, start_index=0,
    ...
    # print('加载数据：', pd1)
    dead_real = pd1['total_dead']  # 死亡人数
    recovery_real = pd1['total_heal']  # 治愈 R
    infectious_real = pd1['total_confirm'] - dead_real - recovery_real  # 感染 I
    susceptible_real = N - dead_real - recovery_real - infectious_real  # 易感 S
    date = pd1['date']  # 日期
    print('-' * 10, 'DataUtil.load_data_SIR_total 结束', '-' * 10)
    return recovery_real + dead_real, infectious_real, susceptible_real, date


# 加载SIR所需要的数据_当日总数据
def load_data_SEIR_total(path='../data/History_country_2020_05_06.csv', name='中国', N=140005000, start_index=0,
                         end_index=0):
    print('-' * 10, 'DataUtil.load_data_SIR_total 开始', '-' * 10)
    print('path:', path)
    print('name:', name)
    ...
    if start_index > end_index:
        print('错误！ 数据行数设置出错，起始行大于结束行。start_index, end_index:', start_index, end_index)
    pd1 = pd.read_csv(path).iloc[start_index:end_index, :].loc[:,
          ('date', 'total_confirm', 'total_dead', 'total_heal', 'total_suspect')]
    print('pd1:',pd1)
    # print('加载数据：', pd1)
    dead_real = pd1['total_dead']  # 死亡人数
    recovery_real = pd1['total_heal']  # 治愈
    suspect = pd1['total_suspect']
    infectious_real = pd1['total_confirm'] - dead_real - recovery_real  # 感染 I
    susceptible_real = N - dead_real - recovery_real - infectious_real  # 易感 S
    date = pd1['date']  # 日期
    print('suspect:', suspect)
    print('-' * 10, 'DataUtil.load_data_SIR_total 结束', '-' * 10)
    return recovery_real + dead_real, suspect, infectious_real, susceptible_real, date
