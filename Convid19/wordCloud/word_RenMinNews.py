# encoding=utf-8
"""
@Time : 2020/3/31 10:59 
@Author : LiuYanZhe
@File : word_RenMinNews.py 
@Software: PyCharm
@Description: 生成人民网50天头条新闻词云图
"""
# encoding=utf-8
...
data0_pd = pd.read_csv('../data/RenMIn_top_2020_03_30.csv')
data_arr = data0_pd.to_numpy().flatten()  # 转换为1为np类型

dic_keyWord_weight = {}
for sentence in data_arr:
    words = TextRank4Keyword()
    words.analyze(text=sentence, lower=True, window=3)
    keywords_list = words.get_keywords(20, word_min_len=2)
...
# print(keywords_pd)
# Util.save_data(keywords_pd, name='RenMin_keyWords_data')
...
color_mask = imread('word.jpg')
# color_mask = imread('wordCloud/python.png')
# 自带的stopwords关键词不起作用，直接删除字典中的词


stopwords = ['现在', '习近平', '总书记', '工作', '主席', '加强', '推动', '强化', '促进', '表示', '历史', '思想', '核心', '吕有金', '任务', '关系', '致以', '地区', '没有', '知道']
...
word_cloud.to_file("../pic/wordCloud_news.png")  # 保存图片
#  显示词云图片
# 从背景图片生成颜色值
