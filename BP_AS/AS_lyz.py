    ...
    while item < MAXITEM:
        ...
        for i in range(ANT_NUM):
            ...
            if E < best_E:
                ...
                # 最优解备份到txt文件中
                np.savetxt('ant_output_threshold_list.txt', best_output_threshold_list)
                np.savetxt('ant_hidden_threshold_list.txt', best_hidden_threshold_list)
                np.savetxt('ant_outhid_weight.txt', best_outhid_weight)
                np.savetxt('ant_inHid_weight.txt', best_inHid_weight)
        ...
        if item % 50 == 0:
            ...
            logging.debug('第' + str(item) + '次最优解E:' + str(best_E))
        list_bestE.append(best_E)
        # logging.debug('信息素：' + str(T_LIST))
        np.savetxt('信息素.txt', T_LIST)
        np.savetxt('蚁群算法每次最优_E.txt', list_bestE)
        item += 1
    # print('最优解E:', best_E)
    logging.debug('最优解E:' + str(best_E))
