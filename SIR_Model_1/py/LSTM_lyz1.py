# encoding=utf-8
"""
@Time : 2020/4/26 20:59 
@Author : LiuYanZhe
@File : LSTM_lyz1.py 
@Software: PyCharm
@Description: 使用长短时记忆网络预测参数
"""
...
def LSTM_lyz(train_seq, add_t, n_steps=3, loop_num=300):
    raw_seq = train_seq
    # 按时间步划分为输入和输出
    ...
    # 训练模型
    model.fit(X, y, epochs=loop_num, batch_size=1, verbose=2)  # 迭代次数，批次数，verbose决定是否显示每次迭代
...
#     # print('预测Y:', yhat_list)
#     pre_t = 10
#     pre_x_input = list(X[len(X) - 1])
#     # 保存预测的Y
#     pre_Y = list(yhat_list)
#     for i in range(3, pre_t + 3, 1):
#         temp_x_input = array(pre_x_input[i - 3:])
#         print('预测X_', i, temp_x_input)
#         temp_x_input = temp_x_input.reshape((1, n_steps, n_features))
#         temp_yhat = model.predict(temp_x_input, verbose=0)
#         print('预测Y_', i, temp_yhat)
...
#     pre_Y = np.array(pre_Y).flatten()
#     print(pre_Y)
#     picUtil.draw_two(raw_seq, pre_Y, 'real', 'pre')
