...
pd.set_option('display.max_rows', None)  # 输出所有行数据
import xlrd
import xlwt
...
class Apriori(object):
    def __init__(self, itemSets, minSupport=0.5, minConf=0.7, sort=False):
        self.itemSets = itemSets  # 标准化项集
        self.minSupport = minSupport  # 最小支持度
        self.minConf = minConf  # 最小置信度
        self.sort = sort
        self.__Initialize()

    def __Initialize(self):  # 初始化
        self.__item()  # 项目
        self.__creat_matrix()
        self.update(minSupport=self.minSupport, minConf=self.minConf)  # 最小支持度和置信度
    ...
    def get_Recommend(self, itemSet, **kwargs):
        ...
        if kwargs.get('file_name'):
            file_name = kwargs['file_name']
            # excel_writer = pd.ExcelWriter(f'{os.getcwd()}/C:\python\PyCharm_workSpace\test/{file_name}.xlsx')
            # path = f'{os.getcwd()}/../table/{file_name}.xlsx'
            path = f'{os.getcwd()}\\{file_name}.xlsx'
            print('path:', path)
...
if __name__ == '__main__':
    ...
    # ap.get_Recommend('2BYP206,2BYW001-'.split(','))
    ap.get_Recommend('2BYP206,2BYW001-,2BYW013,2BYX029'.split(','), file_name='name1')
